...
class GroqProvider(AIProvider):
    ...
    async def health_check(self) -> bool:
        endpoint = f"{self._base_url}{GROQ_CHAT_ENDPOINT}"
    ...
    async def generate(self, prompt_context: PromptContext) -> AIResponse:
        ...
        logger.info(
            "Groq generation started | model=%s | prompt_length=%d",
            self._model,
            len(str(prompt_context)),
        )
        ...
        try:
            ...
            return AIResponse(
                ...
                provider="groq",
                model=self._model,
                errors=["Connection error — provider unreachable"],
                latency=time.monotonic() - start,
            )
        ...
        if resp.status_code in (401, 403):
            logger.warning("Groq auth failure (HTTP %d)", resp.status_code)
            return AIResponse(
                success=False,
                provider="groq",
                model=self._model,
                errors=["Authentication failed — check API key"],
                latency=time.monotonic() - start,
            )
        if resp.status_code == 429:
            logger.warning("Groq rate limit hit (HTTP 429)")
            return AIResponse(
                success=False,
                provider="groq",
                model=self._model,
        ...
        if resp.status_code != 200:
            body = resp.text[:500]
            logger.warning("Groq returned HTTP %d: %s", resp.status_code, body)
            return AIResponse(
                success=False,
                provider="groq",
                model=self._model,
                errors=[f"Provider returned HTTP {resp.status_code}"],
                latency=time.monotonic() - start,
            )
        ...
        try:
            ...
            return AIResponse(
                ...
                provider="groq",
                model=self._model,
                errors=["Invalid JSON in provider response"],
                latency=time.monotonic() - start,
            )
        ...
        try:
            ...
            return AIResponse(
                ...
                provider="groq",
                model=self._model,
                errors=["Unexpected response structure from provider"],
                latency=time.monotonic() - start,
            )
        ...
        latency = time.monotonic() - start
        logger.info(
            "Groq generation finished | model=%s | latency=%.2fs | tokens=%s",
            self._model,
            latency,
    ...
    def _build_messages(prompt_context: PromptContext) -> List[dict]:
        messages = [
            {"role": "system", "content": prompt_context.system_context},
        ]
        body_parts = [
