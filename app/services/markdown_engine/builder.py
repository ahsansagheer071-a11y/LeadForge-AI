...
class MarkdownBuilder:
    ...
    def build_layout_md(self) -> MarkdownDocument:
        ...
        for sec in wl.sections:
            ...
            if sec.description:
                desc = sec.description[:120] + "…" if len(sec.description) > 120 else sec.description
                sec_lines.append(f"- **Description**: {desc}")
    ...
    def build_components_md(self) -> MarkdownDocument:
        ...
        for s in service_sections:
            ...
            if s.description:
                svc_lines.append(f"- **Description**: {s.description[:100]}")
        ...
        for s in testimonial_sections:
            ...
            if s.description:
                test_lines.append(f"- **Description**: {s.description[:100]}")
        ...
        for s in faq_sections:
            ...
            if s.description:
                faq_lines.append(f"  - **Description**: {s.description[:100]}")
        ...
        meta = (
            "\n---\n"
            "Generated by LeadForge AI Premium Website Generator\n"
            f"Document Version: {MARKDOWN_PACKAGE_VERSION}\n"
            f"Generated At: {now.isoformat()}\n"
            "Category: Components\n"
            "Source: WebsiteBlueprint"
        )

        content = "# Website Components\n\n" + "\n\n".join(sections_list) + meta
        content = sanitize_markdown(content)
        content = normalize_headings(content)
        content = normalize_spacing(content)
    ...
    def build_performance_md(self) -> MarkdownDocument:
        ...
        if not has_raw_size:
            content = (
                ...
                "\n\n"
                "---\n"
                f"Generated by LeadForge AI Premium Website Generator\n"
                f"Document Version: {MARKDOWN_PACKAGE_VERSION}\n"
    ...
    def build_package(self) -> MarkdownPackage:
        ...
        logger.info(
            "Package generation complete: %d/12 successful, "
            "%d failed in %.2fs",
